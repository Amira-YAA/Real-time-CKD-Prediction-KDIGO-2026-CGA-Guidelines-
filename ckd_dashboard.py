...
# ============================================================================
# DATA LOADING (CACHED) - SILENT VERSION
# ============================================================================

@st.cache_data
def load_enhanced_data():
    """Load the enhanced dataset with CKD_Stage_CGA column - no messages"""
    try:
        df = pd.read_csv('C:/Users/USER/Downloads/CKD_Dataset_Enhanced.csv')
        return df
...
def predict_ckd_stage(input_data, data, models):
    ...
    return {
        ...
        # Add kidney damage details for display
...
# ============================================================================
# TAB 1: PREDICT CKD - WITH CLINICAL PARAMETERS DASHBOARD (FIXED)
# ============================================================================
...
with tab1:
    ...
    if st.button("🔍 Predict CKD Stage (KDIGO 2026)", type="primary"):
        ...
        # KIDNEY DAMAGE ASSESSMENT SECTION (Using prediction function results)
        st.subheader("🔍 Kidney Damage Assessment")
        
        # Get kidney damage details from result
        kd_details = result.get('kidney_damage_details', {})
        kidney_damage_factors = []
        kidney_damage_present = result['has_kidney_damage']
        
        # Build list of damage factors based on the detailed flags
